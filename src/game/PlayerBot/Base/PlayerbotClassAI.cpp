...
CombatManeuverReturns PlayerbotClassAI::DoManaDrainTask(Unit*) {return RETURN_NO_ACTION_OK;}
CombatManeuverReturns PlayerbotClassAI::DoTankTask(Unit*) { return RETURN_NO_ACTION_OK; }
CombatManeuverReturns PlayerbotClassAI::DoNeutralizeTask(Unit*) { return RETURN_NO_ACTION_OK; }
CombatManeuverReturns PlayerbotClassAI::DoAOETask(Unit*) { return RETURN_NO_ACTION_OK; }

CombatManeuverReturns PlayerbotClassAI::DoNextCombatManeuver(Unit* pTarget)
{ 
	...
	{
		ObjectGuid attackOnGuid = m_bot->GetGroup()->GetTargetIcon(m_ai->m_IconToWatch);
		Unit* thingToAttack = nullptr;
		if (attackOnGuid)
		{
			thingToAttack = ObjectAccessor::GetUnit(*m_bot, attackOnGuid);
		}


		if (thingToAttack != nullptr && thingToAttack->isAlive())
		{
			m_ai->FaceTarget(thingToAttack);
			switch (m_ai->m_Tasktype)
			{
			case TASK_TANK:
				return DoTankTask(thingToAttack);
			case TASK_NEUTRALIZE:
				return DoNeutralizeTask(thingToAttack);
			case TASK_MANADRAIN:
				return DoManaDrainTask(thingToAttack);
			case TASK_AOE:
				return DoAOETask(thingToAttack);
			default:
				break;
			}
		}
		...
		{
			ObjectGuid attackOnGuid = m_bot->GetGroup()->GetTargetIcon(7);
			Unit* thingToAttack = nullptr;
			if (attackOnGuid)
			{
				thingToAttack = ObjectAccessor::GetUnit(*m_bot, attackOnGuid);
			}
			if (thingToAttack != nullptr)
			{
				m_ai->FaceTarget(thingToAttack);
				if (thingToAttack->isAlive())
				{
					m_ai->FaceTarget(thingToAttack);
					return DoNextCombatManeuverPVE(thingToAttack);
				}
				else
				{
					m_ai->FaceTarget(pTarget);
